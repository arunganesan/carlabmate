...
public class TraceReplayer implements Runnable {
    ...
    float specStartTime = -1, specEndTime = -1;
    ...
    public void run () {
        ...
        prefs.edit().putString(Load_From_Trace_Key, null)
             .putFloat(Load_From_Trace_Duration_Start, -1)
             .putFloat(Load_From_Trace_Duration_End, -1).putBoolean(ManualChoiceKey, false)
             .commit();

        carlabService.sendBroadcast(new Intent(carlabService, ManualTrigger.class));
    }
}
